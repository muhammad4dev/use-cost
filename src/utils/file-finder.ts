 ...
 */
const DEFAULT_INCLUDES = ['**/*.{js,jsx,ts,tsx,mjs,cjs,vue,svelte}'];
...
export async function findFiles(options: FindFilesOptions = {}): Promise<string[]> {
  const { cwd = process.cwd(), include = DEFAULT_INCLUDES, exclude = DEFAULT_EXCLUDES } = options;
...
export function shouldIgnoreFile(filePath: string): boolean {
  ...
  const ignoredPatterns = ['node_modules', 'dist', 'build', '.git', 'coverage'];
